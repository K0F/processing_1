...
\def\chapterautorefname{chapt}
\def\sectionautorefname{sect}
\def\subsectionautorefname{subsect}
...
\newcommand{\info}[1]{\todo[color=gray!15]{{\em #1}}}
\newcommand{\otazka}[1]{\todo[color=orange!60]{{\em #1}}}
\newcommand{\kof}[1]{\todo[color=green!70]{{\em #1}}}
...
\begin{center}
\includegraphics[scale = 0.65]{imgs/thePainOfFleetingJoy_1.jpg}
\end{center}
...
\subsection{Uspořádání informací}

Text je řazený do jednotlivých kapitol a dále stromově do dvou úrovní podkapitol. Pořadí kapitol by mělo odpovídat procesu učení se novému programovacímu jazyku.

Pořadí a obsah jednotlivých kapitol vychází ze zkušenosti s vlastním učením se, a zároveň z veliké míry koresponduje s podobnými oficiálními průvodci psanými tvůrci a komunitou kolem programovacího jazyka Processing.

Zvolená forma textu odpovídá již ověřeným postupům. Následující text bude podléhat určitým zákonitostem. V knize se objeví několik typů textu, které budou vždy odlišeny stylem zápisu.
...
\item
Obyčejný text --- člověku srozumitelný popis ve formě textu.
...
\item
Shrnutí důležitých nových informací \info{Poznámky na okraji pro shrnutí nebo zdůraznění informace budou v šedé bublině} v textu.

\item
Kontrolní otázky \otazka{Jakou barvu má bublina značící kontrolní otázku?} v textu.
...
Hovořím-li o stroji, mám dnes na mysli spotřební počítač. Termín \textbf{stroj}\index{stroj} používám záměrně pro zdůraznění jisté formy strojového přemýšlení v historickém kontextu. Stroje jsou 
