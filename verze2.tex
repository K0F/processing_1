...
Interakce se dá definovat vlastně jako animace\footnote{tedy program trvající v čase} s přímým vstupem uživatele do proměnných, tedy do obsahu našich hodnot. Interakce je do jisté míry při práci s počítačem pozorovatelná neustále. Pouhý pohyb myši lze považovat za interakci člověka se strojem. Pohyb uživatele přímo proměňuje hodnoty vykreslující pozici kurzoru, stiskem klávesy opět například interaktivně píšeme text atd.
...
void draw(){
	background(255);
	// v3imn2te si parametr; mouseX a mouseY
