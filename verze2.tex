...
\setsansfont[Scale=1,Mapping=tex-text,Ligatures={Common,Rare,Historical},Numbers={OldStyle}]{Calluna Italic}
\setmonofont{Monaco}
...
\item
\klavesy{CONTROL + T} --- klávesové zkratky, psané pomocí \uv{malých kapitálek}

\item
\vyraz{boolean} --- jednotlivé příkazy a oficiální názvy v textu, zařazené do slovníku, psané \uv{kur\-zívou}

\item
\Slovnikpl{výraz ve slovníku} --- ostatní výrazy, jejichž definici můžete nalézt ve slovníku na konci  knihy.

\item
Shrnutí důležitých nových informací \bublina{Poznámky na okraji pro shrnutí nebo zdůraznění informace budou v šedé bublině} v textu.
...
K otevřenosti v kódu v neposlední řadě přistupuje i celá řada velmi zkušených tvůrců. Tím se uživatel na jakémkoli stupni znalostí může kdykoli naučit nové postupy nebo může svobodně recyklovat algoritmy druhých uživatelů. Processing a čím dál více jeho uživatelů ctí filosofii otevřeného softwaru která (mimo jiné) hlásá: \uv{\em Vědomosti nesmí být privatizovány!}
...
\index{animace}
Statická kresba na plátno je jen jeden z možných výstupů. Animace v Processingu znamená překreslovat plátno pokaždé jinými obrazy. Processing je právě silný v pohyblivém obraze. Nyní si ukážeme základní postup při tvorbě pohyblivého obrazu.

Základní funkce pro vyvolání překreslování výstupního okna je funkce \vyraz{draw()}.
