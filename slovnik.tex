...
\newglossaryentry{acos()}{
	name={acos()},
	description={The inverse of {\textbf cos()}, returns the arc cosine of a value. This function expects the values in the range of -1 to 1 and values are returned in the range {\textbf 0} to {\textbf PI (3.1415927)}.}
}
...
\newglossaryentry{asin()}{
	name={asin()},
	description={The inverse of {\textbf sin()}, returns the arc sine of a value. This function expects the values in the range of -1 to 1 and values are returned in the range {\textbf -PI/2} to {\textbf PI/2}.}
}
...
\newglossaryentry{atan2()}{
	name={atan2()},
	description={Calculates the angle (in radians) from a specified point to the coordinate origin as measured from the positive x-axis. Values are returned as a {\textbf float} in the range from {\textbf PI} to {\textbf -PI}. The {\textbf atan2()} function is most often used for orienting geometry to the position of the cursor.  Note: The y-coordinate of the point is the first parameter and the x-coordinate is the second due the the structure of calculating the tangent.}
}
...
\newglossaryentry{beginRecord()}{
	name={beginRecord()},
	description={Opens a new file and all subsequent drawing functions are echoed to this file as well as the display window. The {\textbf beginRecord()} function requires two parameters, the first is the renderer and the second is the file name. This function is always used with {\textbf endRecord()} to stop the recording process and close the }
}
\newglossaryentry{beginShape()}{
	name={beginShape()},
	description={Using the {\textbf beginShape()} and {\textbf endShape()} functions allow creating more complex forms. {\textbf beginShape()} begins recording vertices for a shape and {\textbf endShape()} stops recording. The value of the {\textbf MODE} parameter tells it which types of shapes to create from the provided vertices. With no mode specified, the shape can be any irregular polygon. The parameters available for beginShape() are POINTS, LINES, TRIANGLES, TRIANGLE_FAN, TRIANGLE_STRIP, QUADS, and QUAD_STRIP. After calling the {\textbf beginShape()} function, a series of {\textbf vertex()} commands must follow. To stop drawing the shape, call {\textbf endShape()}. The {\textbf vertex()} function with two parameters specifies a position in 2D and the {\textbf vertex()} function with three parameters specifies a position in 3D. Each shape will be outlined with the current stroke color and filled with the fill co}
}
...
\newglossaryentry{class}{
	name={class},
	description={Keyword used to indicate the declaration of a class. A class is a composite of data and methods (functions) which may be instantiated as objects. The first letter of a class name is usually uppercase to separate it from other kinds of variables. A related tutorial on <a href="http://java.sun.com/docs/books/tutorial/java/concepts/index.html" target="_blank">Object-Oriented Programming</a> is hosted from the Sun website.}
}
...
\newglossaryentry{color()}{
	name={color()},
	description={Creates colors for storing in variables of the {\textbf color} datatype. The parameters are interpreted as RGB or HSB values depending on the current {\textbf colorMode()}. The default mode is RGB values from 0 to 255 and therefore, the function call {\textbf color(255, 204, 0)} will return a bright yellow color. More about how colors are stored can be found in the reference for the <a href="color_datatype.html">color</a> datatype.}
}
\newglossaryentry{colorMode()}{
	name={colorMode()},
	description={Changes the way Processing interprets color data. By default, the parameters for {\textbf fill()}, {\textbf stroke()}, {\textbf background()}, and {\textbf color()} are defined by values between 0 and 255 using the RGB color model. The {\textbf colorMode()} function is used to change the numerical range used for specifying colors and to switch color systems. For example, calling {\textbf colorMode(RGB, 1.0)} will specify that values are specified between 0 and 1. The limits for defining colors are altered by setting the parameters range1, range2, range3, and range 4.}
}
...
\newglossaryentry{cos()}{
	name={cos()},
	description={Calculates the cosine of an angle. This function expects the values of the {\textbf angle} parameter to be provided in radians (values from 0 to PI*2). Values are returned in the range -1 to 1.}
}
...
\newglossaryentry{ellipse()}{
	name={ellipse()},
	description={Draws an ellipse (oval) in the display window. An ellipse with an equal {\textbf width} and {\textbf height} is a circle. The first two parameters set the location, the third sets the width, and the fourth sets the height. The origin may be changed with the {\textbf ellipseMode()} function.}
}
\newglossaryentry{ellipseMode()}{
	name={ellipseMode()},
	description={The origin of the ellipse is modified by the {\textbf ellipseMode()} function. The default configuration is {\textbf ellipseMode(CENTER)}, which specifies the location of the ellipse as the center of the shape. The RADIUS mode is the same, but the width and height parameters to {\textbf ellipse()} specify the radius of the ellipse, rather than the diameter. The CORNER mode draws the shape from the upper-left corner of its bounding box. The CORNERS mode uses the four parameters to {\textbf ellipse()} to set two opposing corners of the ellipse's bounding box. The parameter must be written in "ALL CAPS" because Processing is a case sensitive language.}
}
\newglossaryentry{else}{
	name={else},
	description={Extends the {\textbf if()} structure allowing the program to choose between two or more block of code. It specifies a block of code to execute when the expression in {\textbf if()} is {\textbf false}.}
}
...
\newglossaryentry{endRecord()}{
	name={endRecord()},
	description={Stops the recording process started by {\textbf beginRecord()} and closes the file.}
}
\newglossaryentry{endShape()}{
	name={endShape()},
	description={The {\textbf endShape()} function is the companion to {\textbf beginShape()} and may only be called after {\textbf beginShape()}. When {\textbf endshape()} is called, all of image data defined since the previous call to {\textbf beginShape()} is written into the image buffer. The constant CLOSE as the value for the MODE parameter to close the shape (to connect the beginning and the end).}
}
...
\newglossaryentry{exit()}{
	name={exit()},
	description={Quits/stops/exits the program. Programs without a {\textbf draw()} function exit automatically after the last line has run, but programs with {\textbf draw()} run continuously until the program is manually stopped or {\textbf exit()} is }
}
...
\newglossaryentry{fill()}{
	name={fill()},
	description={Sets the color used to fill shapes. For example, if you run {\textbf fill(204, 102, 0)}, all subsequent shapes will be filled with orange. This color is either specified in terms of the RGB or HSB color depending on the current {\textbf colorMode()} (the default color space is RGB, with each value in the range from 0 to 2}
}
...
\newglossaryentry{for}{
	name={for},
	description={Controls a sequence of repetitions. A {\textbf for} structure has three parts: {\textbf init}, {\textbf test}, and {\textbf update}. Each part must be separated by a semi-colon ";". The loop continues until the test evaluates to {\textbf false}. When a {\textbf for} structure is executed, the following sequence of events occurs:1. The init statement is executed2. The test is evaluated to be true or false3. If the test is true, jump to step 4. If the test is False, jump to step 64. Execute the statements within the block5. Execute the update statement and jump to step 26. Exit the loop.}
}
\newglossaryentry{frameRate()}{
	name={frameRate()},
	description={Specifies the number of frames to be displayed every second. If the processor is not fast enough to maintain the specified rate, it will not be achieved. For example, the function call {\textbf frameRate(30)} will attempt to refresh 30 times a second. It is recommended to set the frame rate within {\textbf setup()}. The default rate is 60 frames per second.}
}
\newglossaryentry{frameRate}{
	name={frameRate},
...
\newglossaryentry{if}{
	name={if},
	description={Allows the program to make a decision about which code to execute. If the {\textbf test} evaluates to {\textbf true}, the statements enclosed within the block are executed and if the {\textbf test} evaluates to {\textbf false} the statements are not executed.}
}
\newglossaryentry{image()}{
	name={image()},
	description={Displays images to the screen. The images must be in the sketch's "data" directory to load correctly. Select "Add file..." from the "Sketch" menu to add the image. Processing currently works with GIF, JPEG, and Targa images. The color of an image may be modified with the {\textbf tint()} function and if a GIF has transparency, it will maintain its transparency. The {\textbf img} parameter specifies the image to display and the {\textbf x} and {\textbf y} parameters define the location of the image from its upper-left corner. The image is displayed at its original size unless the {\textbf width} and {\textbf height} parameters specify a different size. The {\textbf imageMode()} function changes the way the parameters work. A call to {\textbf imageMode(CORNERS)} will change the width and height parameters to define the x and y values of the opposite corner of the i}
}
\newglossaryentry{imageMode()}{
	name={imageMode()},
	description={Modifies the location from which images draw. The default mode is {\textbf imageMode(CORNER)}, which specifies the location to be the upper left corner and uses the fourth and fifth parameters of {\textbf image()} to set the image's width and height. The syntax {\textbf imageMode(CORNERS)} uses the second and third parameters of {\textbf image()} to set the location of one corner of the image and uses the fourth and fifth parameters to set the opposite corner. Use {\textbf imageMode(CENTER)} to draw images centered at the given x and y posi}
}
...
\newglossaryentry{import}{
	name={import},
	description={The keyword {\textbf import} is used to load a library into a Processing ske}
}
...
\newglossaryentry{keyCode}{
	name={keyCode},
	description={The variable {\textbf keyCode} is used to detect special keys such as the UP, DOWN, LEFT, RIGHT arrow keys and ALT, CONTROL, SHIFT. When checking for these keys, it's first necessary to check and see if the key is coded. This is done with the conditional "if (key == CODED)" as shown in the exam}
}
\newglossaryentry{key}{
	name={key},
	description={The system variable {\textbf key} always contains the value of the most recent key on the keyboard that was used (either pressed or releas}
}
\newglossaryentry{keyPressed()}{
	name={keyPressed()},
	description={The {\textbf keyPressed()} function is called once every time a key is pressed. The key that was pressed is stored in the {\textbf key} varia}
}
\newglossaryentry{keyPressed}{
	name={keyPressed},
...
\newglossaryentry{keyReleased()}{
	name={keyReleased()},
	description={The {\textbf keyReleased()} function is called once every time a key is released. The key that was released will be stored in the {\textbf key} variable. See {\textbf key} and {\textbf keyReleased} for more information.}
}
...
\newglossaryentry{line()}{
	name={line()},
	description={Draws a line (a direct path between two points) to the screen. The version of {\textbf line()} with four parameters draws the line in 2D.  To color a line, use the {\textbf stroke()} function. A line cannot be filled, therefore the {\textbf fill()} method will not affect the color of a line. 2D lines are drawn with a width of one pixel by default, but this can be changed with the {\textbf strokeWeight()} function. The version with six parameters allows the line to be placed anywhere within XYZ space. Drawing this shape in 3D using the {\textbf z} parameter requires the P3D or OPENGL parameter in combination with size as shown in the above example.}
}
...
\newglossaryentry{loadImage()}{
	name={loadImage()},
	description={Loads an image into a variable of type {\textbf PImage}. Four types of images ( {\textbf .gif}, {\textbf .jpg}, {\textbf .tga}, {\textbf .png}) images may be loaded. To load correctly, images must be located in the data directory of the current sketch. In most cases, load all images in {\textbf setup()} to preload them at the start of the program. Loading images inside {\textbf draw()} will reduce the speed of a prog}
}
\newglossaryentry{loadShape()}{
	name={loadShape()},
	description={Loads vector shapes into a variable of type {\textbf PShape}. Currently, only SVG files may be loaded. To load correctly, the file must be located in the data directory of the current sketch. In most cases, {\textbf loadShape()} should be used inside {\textbf setup()} because loading shapes inside {\textbf draw()} will reduce the speed of a ske}
}
\newglossaryentry{loadStrings()}{
	name={loadStrings()},
	description={Reads the contents of a file or url and creates a String array of its individual lines. If a file is specified, it must be located in the sketch's "data" directory/fo}
}
\newglossaryentry{log()}{
	name={log()},
...
\newglossaryentry{mouseButton}{
	name={mouseButton},
	description={Processing automatically tracks if the mouse button is pressed and which button is pressed. The value of the system variable {\textbf mouseButton} is either {\textbf LEFT}, {\textbf RIGHT}, or {\textbf CENTER} depending on which button is pressed.}
}
\newglossaryentry{mouseClicked()}{
	name={mouseClicked()},
	description={The {\textbf mouseClicked()} function is called once after a mouse button has been pressed and then released.}
}
\newglossaryentry{mouseDragged()}{
	name={mouseDragged()},
	description={The {\textbf mouseDragged()} function is called once every time the mouse moves and a mouse button is pressed.}
}
\newglossaryentry{mouseMoved()}{
	name={mouseMoved()},
	description={The {\textbf mouseMoved()} function is called every time the mouse moves and a mouse button is not pressed.}
}
\newglossaryentry{mousePressed()}{
	name={mousePressed()},
	description={The {\textbf mousePressed()} function is called once after every time a mouse button is pressed. The {\textbf mouseButton} variable (see the related reference entry) can be used to determine which button has been pressed.}
}
\newglossaryentry{mousePressed}{
	name={mousePressed},
...
\newglossaryentry{mouseReleased()}{
	name={mouseReleased()},
	description={The {\textbf mouseReleased()} function is called every time a mouse button is released.}
}
...
\newglossaryentry{new}{
	name={new},
	description={Creates a "new" object. The keyword {\textbf new} is typically used similarly to the applications in the above example. In this example, a new object "h1" of the datatype "HLine" is created. On the following line, a new array of floats called "speeds" is created.}
}
\newglossaryentry{nfc()}{
	name={nfc()},
...
\newglossaryentry{nf()}{
	name={nf()},
	description={Utility function for formatting numbers into strings. There are two versions, one for formatting floats and one for formatting ints. The values for the {\textbf digits}, {\textbf left}, and {\textbf right} parameters should always be positive integers.As shown in the above example, {\textbf nf()} is used to add zeros to the left and/or right of a number. This is typically for aligning a list of numbers. To <em>remove</em> digits from a floating-point number, use the {\textbf int()}, {\textbf ceil()}, {\textbf floor()}, or {\textbf round()} functions.}
}
\newglossaryentry{nfp()}{
	name={nfp()},
...
\newglossaryentry{noFill()}{
	name={noFill()},
	description={Disables filling geometry. If both {\textbf noStroke()} and {\textbf noFill()} are called, nothing will be drawn to the screen.}
}
...
\newglossaryentry{noise()}{
	name={noise()},
	description={Returns the Perlin noise value at specified coordinates. Perlin noise is a random sequence generator producing a more natural ordered, harmonic succession of numbers compared to the standard {\textbf random()} function. It was invented by Ken Perlin in the 1980s and been used since in graphical applications to produce procedural textures, natural motion, shapes, terrains etc. The main difference to the {\textbf random()} function is that Perlin noise is defined in an infinite n-dimensional space where each pair of coordinates corresponds to a fixed semi-random value (fixed only for the lifespan of the program). The resulting value will always be between 0.0 and 1.0. Processing can compute 1D, 2D and 3D noise, depending on the number of coordinates given. The noise value can be animated by moving through the noise space as demonstrated in the example above. The 2nd and 3rd dimension can also be interpreted as time.The actual noise is structured similar to an audio signal, in respect to the function's use of frequencies. Similar to the concept of harmonics in physics, perlin noise is computed over several octaves which are added together for the final result. Another way to adjust the character of the resulting sequence is the scale of the input coordinates. As the function works within an infinite space the value of the coordinates doesn't matter as such, only the distance between successive coordinates does (eg. when using {\textbf noise()} within a loop). As a general rule the smaller the difference between coordinates, the smoother the resulting noise sequence will be. Steps of 0.005-0.03 work best for most applications, but this will differ depending on use.}
}
...
\newglossaryentry{noStroke()}{
	name={noStroke()},
	description={Disables drawing the stroke (outline). If both {\textbf noStroke()} and {\textbf noFill()} are called, nothing will be drawn to the screen.}
}
...
\newglossaryentry{PImage}{
	name={PImage},
	description={Datatype for storing images. Processing can display {\textbf .gif}, {\textbf .jpg}, {\textbf .tga}, and {\textbf .png} images. Images may be displayed in 2D and 3D space. Before an image is used, it must be loaded with the {\textbf loadImage()} function. The {\textbf PImage} object contains fields for the {\textbf width} and {\textbf height} of the image, as well as an array called {\textbf pixels[]} which contains the values for every pixel in the image. A group of methods, described below, allow easy access to the image's pixels and alpha channel and simplify the process of composit}
}
...
\newglossaryentry{sin()}{
	name={sin()},
	description={Calculates the sine of an angle. This function expects the values of the {\textbf angle} parameter to be provided in radians (values from 0 to 6.28). Values are returned in the range -1 to 1.}
}
\newglossaryentry{size()}{
	name={size()},
	description={Defines the dimension of the display window in units of pixels. The {\textbf size()} function <em>must</em> be the first line in {\textbf setup()}. If {\textbf size()} is not called, the default size of the window is 100x100 pixels. The system variables {\textbf width} and {\textbf height} are set by the parameters passed to the {\textbf size()} function. <}
}
\newglossaryentry{smooth()}{
	name={smooth()},
	description={Draws all geometry with smooth (anti-aliased) edges. This will slow down the frame rate of the application, but will enhance the visual refinem}
}
...
\newglossaryentry{vertex()}{
	name={vertex()},
	description={All shapes are constructed by connecting a series of vertices. {\textbf vertex()} is used to specify the vertex coordinates for points, lines, triangles, quads, and polygons and is used exclusively within the {\textbf beginShape()} and {\textbf endShape()} function. Drawing a vertex in 3D using the {\textbf z} parameter requires the P3D or OPENGL parameter in combination with size as shown in the above example.This function is also used to map a texture onto the geometry. The {\textbf texture()} function declares the texture to apply to the geometry and the {\textbf u} and {\textbf v} coordinates set define the mapping of this texture to the form. By default, the coordinates used for {\textbf u} and {\textbf v} are specified in relation to the image's size in pixels, but this relation can be changed with {\textbf textureMode()}.}
}
\newglossaryentry{void}{
	name={void},
